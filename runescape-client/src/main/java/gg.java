import java.io.EOFException;
import net.runelite.api.annotations.ObfuscatedName;
import net.runelite.api.annotations.ObfuscatedSignature;

@ObfuscatedName("gg")
public interface gg {
	@ObfuscatedName("ao")
	@ObfuscatedSignature(
		descriptor = "(II)V"
	)
	void ao(int var1, int var2);

	@ObfuscatedName("ag")
	@ObfuscatedSignature(
		descriptor = "(Llu;III)V"
	)
	void ag(lu var1, int var2, int var3, int var4) throws EOFException;

	@ObfuscatedName("au")
	@ObfuscatedSignature(
		descriptor = "(IIIIID)V"
	)
	void au(int var1, int var2, int var3, int var4, int var5, double var6) throws EOFException;

	@ObfuscatedName("ak")
	@ObfuscatedSignature(
		descriptor = "(IIIII)V"
	)
	void ak(int var1, int var2, int var3, int var4, int var5) throws EOFException;

	@ObfuscatedName("az")
	@ObfuscatedSignature(
		descriptor = "(IIIIID)V"
	)
	void az(int var1, int var2, int var3, int var4, int var5, double var6) throws EOFException;

	@ObfuscatedName("ah")
	@ObfuscatedSignature(
		descriptor = "(IIII)V"
	)
	void ah(int var1, int var2, int var3, int var4) throws EOFException;

	@ObfuscatedName("aw")
	@ObfuscatedSignature(
		descriptor = "(IIII)V"
	)
	void aw(int var1, int var2, int var3, int var4) throws EOFException;

	@ObfuscatedName("an")
	@ObfuscatedSignature(
		descriptor = "(IIIIID)V"
	)
	void an(int var1, int var2, int var3, int var4, int var5, double var6) throws EOFException;

	@ObfuscatedName("al")
	@ObfuscatedSignature(
		descriptor = "(IIIIID)V"
	)
	void al(int var1, int var2, int var3, int var4, int var5, double var6) throws EOFException;

	@ObfuscatedName("ax")
	@ObfuscatedSignature(
		descriptor = "(IIIIID)V"
	)
	void ax(int var1, int var2, int var3, int var4, int var5, double var6) throws EOFException;

	@ObfuscatedName("as")
	@ObfuscatedSignature(
		descriptor = "(Llu;II)V"
	)
	void as(lu var1, int var2, int var3) throws EOFException;

	@ObfuscatedName("ae")
	@ObfuscatedSignature(
		descriptor = "(IIB)V"
	)
	void ae(int var1, int var2, byte var3);

	@ObfuscatedName("af")
	@ObfuscatedSignature(
		descriptor = "(Llu;II)V"
	)
	void af(lu var1, int var2, int var3) throws EOFException;

	@ObfuscatedName("av")
	@ObfuscatedSignature(
		descriptor = "(IIIII)V"
	)
	void av(int var1, int var2, int var3, int var4, int var5) throws EOFException;

	@ObfuscatedName("ar")
	@ObfuscatedSignature(
		descriptor = "(Llu;II)V"
	)
	void ar(lu var1, int var2, int var3) throws EOFException;

	@ObfuscatedName("aa")
	@ObfuscatedSignature(
		descriptor = "(IIIIID)V"
	)
	void aa(int var1, int var2, int var3, int var4, int var5, double var6) throws EOFException;

	@ObfuscatedName("ai")
	@ObfuscatedSignature(
		descriptor = "(IIII)V"
	)
	void ai(int var1, int var2, int var3, int var4) throws EOFException;

	@ObfuscatedName("aq")
	@ObfuscatedSignature(
		descriptor = "(IIII)V"
	)
	void aq(int var1, int var2, int var3, int var4) throws EOFException;

	@ObfuscatedName("am")
	@ObfuscatedSignature(
		descriptor = "(II)V"
	)
	void am(int var1, int var2);

	@ObfuscatedName("ad")
	@ObfuscatedSignature(
		descriptor = "(II)V"
	)
	void ad(int var1, int var2);

	@ObfuscatedName("ay")
	@ObfuscatedSignature(
		descriptor = "(Llu;II)V"
	)
	void ay(lu var1, int var2, int var3) throws EOFException;

	@ObfuscatedName("ab")
	@ObfuscatedSignature(
		descriptor = "(II)V"
	)
	void ab(int var1, int var2);
}
